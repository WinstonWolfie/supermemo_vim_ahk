...
SMImportButtonImport:
  ...
  SkipDate := (OnlineEl && !IsVideoOrAudioSite)
...
!x::
  ...
  KeyWait Ctrl
  if (!Copy(false)) {
    Vim.State.SetToolTip("Nothing is selected.")
    Goto RestoreClipReturn
  } else {
...
ExtractToSMAgain:
  ...
  RefLink := hBrowser ? Vim.SM.GetLinkFromHTMLAllText(auiaText) : ""
